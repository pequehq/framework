...
export class SwaggerFactory {
  ...
  private static createServersFile() {
    SwaggerFactory.render(
      ...
      getPath(SERVERS_GENERATED_PATH),
      ...
      Object.entries(routesByPath).forEach(
        ([actualPath, actualRoutes]: [string, SwaggerRouteDefinition[]]) => {
          ...
          actualRoutes.forEach(route => {
            ...
            SwaggerFactory.render(PATHS_METHOD_TEMPLATE_PATH, getPath(`${PATHS_GENERATED_FOLDER_PATH}/${controllerName}/${fileName}`), methodObject);
          });
